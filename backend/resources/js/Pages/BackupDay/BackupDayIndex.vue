...
export default {
    ...
    data() {
        return {
            url: '/admin/backup-days-all-paginate',
            columns: [
                'id',
                'name',
                'description',
        ...
        deleteBackupObject(row){
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    axios.delete('/admin/backup-days/' + row.id).then(response => {
                        if (response.data.status === true) {
                            this.$notify({
                                group: 'foo',
                                type: 'success',
                                title: 'Удаление дня бэкапа',
                                text: 'Тип дня успешно удален.'
                            });
                            Swal.fire(
                                'Удалено!',
                                'Тип дня удален',
                            ...
                            this.$refs['days-table'].refresh();
                        }
                        if (response.data.status === false) {
                            this.$notify({
                                group: 'foo',
                                type: 'error',
                                title: 'Удаление типа дня',
                            });
                            Swal.fire(
                                'Ошибка!',
